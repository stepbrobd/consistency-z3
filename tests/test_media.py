...
def test_media() -> None:
    ...
    # admin login (its assumed that admin users are pre-registered)
    ...
    cons_op_types = z3.And(
        op.type(op_client_register) == wr,       # type: ignore
        op.type(op_client_login) == rd,          # type: ignore
        op.type(op_admin_login) == rd,           # type: ignore
        op.type(op_admin_modify) == wr,          # type: ignore
        op.type(op_user_db_rd) == rd,            # type: ignore
        op.type(op_user_db_wr) == wr,            # type: ignore
        op.type(op_client_rent_video) == wr,     # type: ignore
        op.type(op_video_check_metadata) == rd,  # type: ignore
        op.type(op_rent_check_metadata) == rd,   # type: ignore
        op.type(op_review_check_metadata) == rd, # type: ignore
        op.type(op_client_read_review) == rd,    # type: ignore
        op.type(op_client_write_review) == wr,   # type: ignore
        op.type(op_client_watch_video) == rd,    # type: ignore
        op.type(op_admin_write_metadata) == wr,  # type: ignore
        op.type(op_admin_write_video) == wr,     # type: ignore
    )
    ...
    ec_client_rd_login = [(Cons("User must register before login", z3.And(ec_common_login, z3.Exists([op_client_login, op_client_register], z3.And(ob(op_client_login, op_client_register), vis(op_client_register, op_client_login))))),)] # type: ignore
    ec_admin_rd_login = None # admins are pre-registered, no need to assign constraints here
    # admin writes require a previous admin login
    ec_admin_wr_login = [(Cons("Admin writes requires login", ec_common_admin),)] # type: ignore
    # user db reads require a previous write, admin writes must reflect on user reads
    ec_login_rd_user_db = [(Cons("Combined constraints from users and admins", z3.And(z3.Exists([op_user_db_rd, op_user_db_wr], z3.And(ob(op_user_db_rd, op_user_db_wr), vis(op_user_db_wr, op_user_db_rd))), z3.Exists([op_user_db_rd, op_admin_modify], z3.And(ob(op_admin_modify, op_user_db_rd), vis(op_admin_modify, op_user_db_rd))))),)] # type: ignore
    ec_login_wr_user_db = None # assume all user db writes are valid

    ec_client_wr_rent = [(Cons("Client login before rent movie", ec_common_login),)] # type: ignore
    ...
    ec_client_rd_review = [(Cons("Require login before read review", ec_common_login),)]  # type: ignore
    ec_client_wr_review = [(Cons("Require login before write review", ec_common_login),)] # type: ignore
    ...
    ec_client_rd_video = [(Cons("Client watch video", ec_common_login),)] # type: ignore
    ...
    ec_admin_wr_metadata_db = [(Cons("Admin login before write metadata", z3.And(ec_common_admin)),)] # type: ignore
    ec_admin_wr_video = [(Cons("Admin login before update video", z3.And(ec_common_admin)),)] # type: ignore
    ...
    g = graph(nodes, edges)
    ok, res = composable(g, client, cons_op_types) # type: ignore
    assert ok

    for edge in res.edges(keys=True): # type: ignore
        src, dst, _ = edge
